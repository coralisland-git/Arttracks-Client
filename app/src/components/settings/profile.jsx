...
class Profile extends React.Component {
  constructor(...args) {
    ...
    this.state = { avatar: null, status: null, fullName: '', username: '', language: '', current_email: '' };
  }
  ...
  componentWillReceiveProps(newProps) {
    const { fullName, language, mugshot, username, email, status, cmdType } = newProps;
    ...
    this.setState({ avatar: mugshot, fullName, username, language, email });
  ...
  render() {
    ...
    const { status, fullName, username, language, email, current_email } = this.state;
    console.log("")
    ...
    const avatar = this.state.avatar ?
        <img src={ this.state.avatar.replace("http:", "https:") } width='115' height='115' alt='sarah_patchett' /> :
          ...
          <Grid>
            <Row>
                  ...
                  <Row>
                    <Col md={6}>
                      <Button lg type='submit' bsStyle='primary' className="btn-sq btn-block">
                        { status == 'saving' && cmdType == 'profile' ? <ButtonLoader primary={true} />: 'Save' }
                      </Button>
                    </Col>
                  </Row>
                  <hr className="transparent" />
              ...
              <Col md={6} lg={4} lgOffset={1}>
                ...
                <p>Your current email address is:<br /><span className="dark">{current_email}</span></p>
                <hr className="transparent" />
                ...
                <Form onSubmit={::this.saveEmail}>
                  ...
                  <Row>
                    <Col md={6}>
                      <Button lg type='submit' bsStyle='primary' className="btn-sq btn-block">
                        { status == 'saving' && cmdType == 'email' ? <ButtonLoader primary={true} />: 'Save' }
                      </Button>
                    </Col>
                  </Row>
                  <hr className="transparent" />
